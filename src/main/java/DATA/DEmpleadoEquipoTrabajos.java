 ...
 * @author fpl
 */
public class DEmpleadoEquipoTrabajos extends BaseDAO<DEmpleadoEquipoTrabajos> {
    private int id;
    ...
    private static final String TABLE = "empleado_equipo_trabajos";
    private static final String QUERY_ID = "id";
    private static final String QUERY_INSERT = String.format(
            "INSERT INTO %s (estado, ocupacion, created_at, empleado_id, equipo_trabajo_id) VALUES (?,?,?,?,?)", TABLE);
    ...
    private final String QUERY_LIST_ALL = "SELECT * FROM " + TABLE;
    ...
    @Override
    protected String[] entityToStringArray(DEmpleadoEquipoTrabajos entity) {
        ...
        return new String[]{
                ...
                String.valueOf(entity.getEmpleado_id()),
    ...
    @Override
    protected DEmpleadoEquipoTrabajos stringArrayToEntity(String[] data) {
        DEmpleadoEquipoTrabajos modelo = new DEmpleadoEquipoTrabajos();
        try {
            modelo.setId(Integer.parseInt(data[0]));
            modelo.setEmpleado_id(Integer.parseInt(data[1]));
            modelo.setEquipo_trabajo_id(Integer.parseInt(data[2]));
            modelo.setEstado(data[3]);
            modelo.setOcupacion(data[4]);
            modelo.setCreated_at(toLocalDateTime(data[5]));
            modelo.setUpdated_at(toLocalDateTime(data[6]));
    ...
    @Override
    protected void prepareStatementForEntity(DEmpleadoEquipoTrabajos entity) throws SQLException {
        if (ps == null) {
            return;
        }
        // Para INSERT y UPDATE
        ps.setString(1, entity.getEstado());
        ps.setString(2, entity.getOcupacion());
        ...
        if (ps.getParameterMetaData().getParameterCount() == 4) {
            ...
            ps.setString(4, String.valueOf(entity.getEmpleado_id()));
            ps.setString(5, String.valueOf(entity.getEquipo_trabajo_id()));
        }
    }
    @Override
    ...
    public String[] ver() throws SQLException {
        DEmpleadoEquipoTrabajos modelo = findById(this.id);
    ...
    public List<String[]> listar() throws SQLException {
        List<DEmpleadoEquipoTrabajos> modelos = findAll();
        List<String[]> result = new ArrayList<>();
        for (DEmpleadoEquipoTrabajos modelo : modelos) {
